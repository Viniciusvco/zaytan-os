...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "distribute") {
      ...
      return jsonRes({ success: true, distributed, stocked, total_pending: pendingLeads.length });
    }
    ...
    if (action === "send_stock") {
      ...
      for (const lqId of lead_queue_ids) {
        ...
        if (cc?.daily_limit !== null && cc && cc.leads_received_today >= cc.daily_limit) continue;
